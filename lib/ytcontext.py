...
def getDownloadPath(use_default=None):
    if use_default is None:
        use_default = not ADDON.getSetting('confirm_download_path') is True
    path = ADDON.getSetting('last_download_path')
    if path:
        if not use_default:
            new = xbmcgui.Dialog().yesno("Use Default?", "Use default path:", path, "Or choose a new path?", "Default",
                                         "New")
            if new:
                path = ''
    if not path:
...
def ERROR(msg=None, hide_tb=False):
    ...
    if hide_tb and not DEBUG:
        ...
        LOG('%s::%s (%d) - %s' % (
            msg or '?', sys.exc_info()[2].tb_frame.f_code.co_name, sys.exc_info()[2].tb_lineno, errtext))
        return
...
class main():
    ...
    def playlist():
        ...
        if plugin_url:
            ...
            if result:
                xbmc.log('context.youtube.dl.menu: ' + 'Found playlist_id in url: |%s|' % result.group('playlist_id'),
                         xbmc.LOGINFO)
        ...
        if container_url and not playlist_id:
            ...
            if result:
                xbmc.log('context.youtube.dl.menu: ' + 'Found playlist_id in container url: |%s|' % result.group(
                    'playlist_id'), xbmc.LOGINFO)
                playlist_id = result.group('playlist_id')
            else:
                xbmc.log('context.youtube.dl.menu: ' + 'playlist_id not found in container url ' + plugin_url,
                         xbmc.LOGERROR)
        else:
            xbmc.log('context.youtube.dl.menu: ' + 'Container URL not found', xbmc.LOGERROR)

        if not playlist_id:
            xbmcgui.Dialog().notification('youtube-dl error', 'playlist id not found',
        ...
        if xbmc.Player().isPlaying():
            xbmcgui.Dialog().notification('youtube-dl', 'Playlist download started',
                                          os.path.join(addonFolder, "icon.png"), 5000, True)
        ...
        with youtube_dl.YoutubeDL(ydl_opts) as ydl:
            try:
                ...
                xbmcgui.Dialog().notification('youtube-dl error', 'Fatal YTD Error, check logs',
                                              os.path.join(addonFolder, "icon.png"), 5000, True)
                return False

            if ydl_info == None:
                playlistDLProgress.close()
                xbmc.log('context.youtube.dl.menu: ' + 'Fatal error, youtube-dl core may need to be updated.',
                         xbmc.LOGERROR)
                xbmcgui.Dialog().notification('youtube-dl error', 'Could not obtain playlist, check logs',
                                              os.path.join(addonFolder, "icon.png"), 5000, True)
                return False
            ...
            if not continuedownload:
                ...
                playlistDLProgress.close()
                xbmcgui.Dialog().notification('youtube-dl', 'Playlist download aborted',
                                              os.path.join(addonFolder, "icon.png"), 5000, True)
                return False
            else:
                ...
                for entry in ydl_info['entries']:
                    ...
                    if sys.version_info.major == 3:
                        playlistDLProgress.update(int(100 / amt * i), 'Downloading Playlist...',
                                                  'Downloading ' + entry['title'] + ' ...')
                    else:
                        playlistDLProgress.update(int(100 / amt * i), 'Downloading Playlist...',
                                                  'Downloading ' + entry['title'].decode('latin1') + ' ...')
                    try:
                ...
                xbmc.sleep(1000)
                playlistDLProgress.close()
                xbmcgui.Dialog().notification('youtube-dl', 'Playlist download completed',
                                              os.path.join(addonFolder, "icon.png"), 5000, True)
                return True
    ...
    def getYTID():
        ...
        if not ytid:
            ...
            if plugin_url:
                ...
                if result:
                    xbmc.log('context.youtube.dl.menu: ' + 'Found video_id in url: |%s|' % result.group('video_id'),
                             xbmc.LOGINFO)
                    video_id = result.group('video_id')
                else:
                    xbmc.log('context.youtube.dl.menu: ' + 'video_id not found in url ' + plugin_url, xbmc.LOGERROR)
                    xbmcgui.Dialog().notification('youtube-dl error', 'video_id not found in url',
                                                  os.path.join(addonFolder, "icon.png"), 5000, True)
                    return 1
                ...
                xbmc.log('context.youtube.dl.menu: ' + 'Plugin URL not found', xbmc.LOGERROR)
                xbmcgui.Dialog().notification('youtube-dl error', 'url not found',
                                              os.path.join(addonFolder, "icon.png"), 5000, True)
                return 1
    ...
    def download(info=None, isPlaylist=False):
        ...
        if info == None:
            ...
            info = main.getYTID()
            if info == False:
                xbmcgui.Dialog().notification('youtube-dl error', 'Youtube ID could not be obtained',
                                              os.path.join(addonFolder, "icon.png"), 5000, True)
                return False
        ...
        if not xbmcvfs.exists(targetdir):
            ...
            xbmcgui.Dialog().notification('youtube-dl error',
                                          'destination path does not exist, check youtube-dl control configuration',
                                          os.path.join(addonFolder, "icon.png"), 5000, True)
            return False
        ...
        if writeconfirm:
            ...
            xbmcgui.Dialog().notification('youtube-dl error', 'destination path is not writeable',
                                          os.path.join(addonFolder, "icon.png"), 5000, True)
            return False
        ...
        if yes:
            ...
            if xbmc.Player().isPlaying() and not isPlaylist:
                xbmcgui.Dialog().notification('Video download started', title, os.path.join(addonFolder, "icon.png"),
                                              5000, True)
            ...
            if sys.version_info.major == 3:
                ...
                pDialog.create('YoutubeDL ' + title.decode('latin1'),
                               'Preparing to download ' + title.decode('latin1') + ' ...')
            ...
            if sys.version_info.major == 3:
                ydl_opts = {
                    'outtmpl': os.path.join(xbmcvfs.translatePath(
                        'special://temp')) + '%(title)s' + outtmpl_upload_year + outtmpl_resolution + '.%(ext)s',
                    'nopart': True,
                    #                    'forcefilename': True,
                    'logger': MyLogger(),
                }
                ...
                ydl_opts = {
                    'outtmpl': os.path.join(xbmc.translatePath(
                        'special://temp')) + '%(title)s' + outtmpl_upload_year + outtmpl_resolution + '.%(ext)s',
                    'nopart': True,
                    #                    'forcefilename': True,
                    'logger': MyLogger(),
            ...
            if ffm_ver.get("avconv") is False and ffm_ver.get("ffmpeg") is False:
                ydl_opts.update({"format": "best[height<=" + max_video_resolution + "]"})
            else:
                ydl_opts.update({
                    "format": "bestvideo[height<=" + max_video_resolution + "]+bestaudio/best[height<=" + max_video_resolution + "]"})
            ...
            with youtube_dl.YoutubeDL(ydl_opts) as ydl:
                try:
                    ...
                    xbmcgui.Dialog().notification('youtube-dl error', 'YTD JS Error, check logs',
                                                  os.path.join(addonFolder, "icon.png"), 5000, True)
                    return False
                    ...
                    xbmcgui.Dialog().notification('youtube-dl error', '{0}'.format(de),
                                                  os.path.join(addonFolder, "icon.png"), 10000, True)
                    return False
                except:
                ...
                LOG("Predownload: Check if file already exists in target directory " + os.path.join(targetdir,
                                                                                                    os.path.basename(
                                                                                                        filename)),
                    debug=True)
                if xbmcvfs.exists(os.path.join(targetdir, os.path.basename(filename))):
                    LOG("Predownload: File already exists in target directory " + os.path.basename(filename),
                        debug=True)
                    ...
                    if fileinfo_dst_size == 0:
                        LOG("Predownload: Existing file in target directory has filesize of 0, deleting and continue",
                            debug=True)
                        ...
                        continuedownload = xbmcgui.Dialog().yesno('Continue downloading?', os.path.basename(
                            filename) + ' already exists!\nDelete and download again?', '')
                        if not continuedownload:
                            LOG("Predownload: File already exist in target directory, not overwriting as requested",
                                debug=True)
                            pDialog.close()
                            xbmcgui.Dialog().notification('youtube-dl', 'Download cancelled',
                                                          os.path.join(addonFolder, "icon.png"), 5000, True)
                            return False
                        else:
                ...
                if sys.version_info.major == 3:
                    ...
                    try:
                        ...
                        pDialog.close()
                        xbmcgui.Dialog().notification('youtube-dl', 'Download stalled (' + title + ')',
                                                      os.path.join(addonFolder, "icon.png"), 5000, True)
                        return False
                else:
                    ...
                    try:
                        ...
                        LOG("Download stalled", debug=True)
                        pDialog.close()
                        xbmcgui.Dialog().notification('youtube-dl', 'Download stalled (' + title.decode('title1') + ')',
                                                      os.path.join(addonFolder, "icon.png"), 5000, True)
                        return False
                ...
                while True:
                    ...
                    if result is False:
                        ...
                        if not xbmcvfs.exists(filename):
                            ...
                            xbmcgui.Dialog().notification('youtube-dl error', 'missing source file, check logs...',
                                                          os.path.join(addonFolder, "icon.png"), 5000, True)
                            break
                        retry = xbmcgui.Dialog().yesno('Move error',
                                                       'Failed to move ' + filename + ' to destination folder.\n\nTry again?',
                                                       '')
                        if not retry:
                            LOG("abandoned retry", debug=True)
                            delete = xbmcgui.Dialog().yesno('Move error - Delete temp file?',
                                                            'Failed to move\n' + os.path.basename(
                                                                filename) + '\nto ' + targetdir + '\nDelete temp file?',
                                                            '')
                            if delete:
                                ...
                                xbmcgui.Dialog().notification('youtube-dl deleted', title,
                                                              os.path.join(addonFolder, "icon.png"), 5000, True)
                                LOG("breaking post action loop", debug=True)
                            break
                ...
                # cleanup
                if addon_created_new_dir is True and len(xbmcvfs.listdir(targetdir)) == 0:
...
def handleFinished(filename, title, targetdir, isPlaylist):
    ...
    if xbmcvfs.exists(os.path.join(targetdir, os.path.basename(filename))):
        ...
        if fileinfo_dst_size == 0:
            ...
            xbmc.sleep(1000)
            pDialog.close()
            delete = xbmcgui.Dialog().yesno('File exists - Delete temp file?',
                                            'Target file already exists.\n' + os.path.basename(
                                                filename) + '\nDelete temp file?', '')
            if delete:
                xbmcvfs.delete(filename)
                xbmcgui.Dialog().notification('youtube-dl temp deleted', title, os.path.join(addonFolder, "icon.png"),
                                              5000, True)
            return True
        ...
        if xbmcvfs.copy(file_path, destFilePath):
            ...
            if xbmcvfs.exists(destFilePath):
                ...
                if fileinfo_src_size == fileinfo_dst_size:
                    ...
                    LOG("File sizes do not match, copy failed. Source size = " + str(
                        fileinfo_src_size) + ", Destination size = " + str(fileinfo_dst_size), debug=True)
                    return False
            else:
